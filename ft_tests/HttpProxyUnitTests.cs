...
public class HttpProxyUnitTests
{
    ...
    private const int StressPayloadBytes = 8 * 1024;
    ...
    private static (Thread A, Thread B) StartDualHttpProxyTunnel(string aArgs, string bArgs, string writeFilename, string readFilename)
    {
    ...
    private static (Thread Listen, Thread Forward) StartTunnel(string proxyArg, string writeFilename, string readFilename)
    {
    ...
    private static void Teardown(TcpClient? origin, Thread? acceptThread, Thread listenThread, Thread forwardThread, string readFilename, string writeFilename)
    {
    ...
    private static TcpClient ConnectHttp(int proxyPort, string destHost, int destPort)
    {
    ...
    private static TcpClient ConnectToProxyPort(int proxyPort)
    {
    ...
    private static void DrainToBlankLine(NetworkStream stream)
    {
        var matched = 2;
        while (matched < 4)
        {
    ...
    private static void PerformHttpEchoRequest(int proxyPort, int destPort, int requestId)
    {
    ...
    private static void EchoUntilClosed(TcpClient client)
    {
        try
        {
            using (client)
    ...
    private static void WaitForProxyReady(int proxyPort)
    {
        var start = DateTime.Now;
        while (true)
        {
