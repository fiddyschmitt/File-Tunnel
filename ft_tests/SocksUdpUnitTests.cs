...
public class SocksUdpUnitTests
{
    ...
    private static void RunUdpTunnel(int socksPort, int[] destPorts, Action<TcpClient, UdpClient, IPEndPoint, int[]> body)
    {
    ...
    private sealed class EchoServer { public required UdpClient Socket; public volatile bool Stop; }

    private static EchoServer StartUdpEcho(int port)
    {
    ...
    private static IPEndPoint UdpAssociate(TcpClient control)
    {
    ...
    private static byte[] SendAndReceive(UdpClient relaySocket, IPEndPoint bnd, byte[] requestDatagram, out int headerLen)
    {
    ...
    private static int UdpHeaderLength(byte[] datagram)
    {
        var atyp = datagram[3];
    ...
    private static byte[] AddressBytes(byte atyp, string host) => atyp switch
    {
    ...
    private static byte[] BuildRequestDatagram(byte frag, byte atyp, byte[] addr, int port, byte[] payload)
    {
    ...
    private static byte[] BuildUdpDatagram(byte frag, byte atyp, byte[] addr, int port, byte[] data)
        => BuildRequestDatagram(frag, atyp, addr, port, data);

    private static TcpClient ConnectToSocksPort(int socksPort)
    {
    ...
    private sealed class CapturingSink : ISocksUdpSink
    {
    ...
    private sealed class DuplexTestStream(byte[] toRead) : Stream
    {
        private readonly MemoryStream readSide = new(toRead);
        private readonly MemoryStream writeSide = new();
