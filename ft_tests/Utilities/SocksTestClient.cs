...
public static class SocksTestClient
{
    ...
    private static (TcpClient Control, IPEndPoint Bnd) UdpAssociate(IPEndPoint proxy, int timeoutMs)
    {
    ...
    private static byte[] Wrap(byte atyp, byte[] addr, int port, byte[] payload)
    {
    ...
    private static byte[] Unwrap(byte[] datagram)
    {
        var atyp = datagram[3];
    ...
    private static byte[] BuildDnsQuery(ushort txid, string domain)
    {
        var q = new List<byte>
        {
