...
{
    private const long SESSION_ID = 0;
    private const int READY_FOR_PURGE_FLAG = sizeof(long);
    private const int PURGE_COMPLETE_FLAG = READY_FOR_PURGE_FLAG + 1;
    private const int MESSAGE_WRITE_POS = PURGE_COMPLETE_FLAG + 1;

    private ToggleWriter? setReadyForPurge;
    private ToggleWriter? setPurgeComplete;

    private ToggleReader? isReadyForPurge;
    private ToggleReader? isPurgeComplete;
        ...
        while (true)
        {
            ...
            try
            {
                ...
                fileStream = new FileStream(WriteToFilename, FileMode.Create, FileAccess.ReadWrite, FileShare.ReadWrite,
                    bufferSize, FileOptions.WriteThrough); //large buffer to prevent FileStream from autoflushing
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                var setReadyForPurgeStream = new FileStream(WriteToFilename, FileMode.Open, FileAccess.ReadWrite,
                    FileShare.ReadWrite, 1, FileOptions.SequentialScan | FileOptions.WriteThrough);
                ...
                var setPurgeCompleteStream = new FileStream(WriteToFilename, FileMode.Open, FileAccess.ReadWrite,
                    FileShare.ReadWrite, 1, FileOptions.SequentialScan | FileOptions.WriteThrough);
                ...
                while (true)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (SendQueue.TryTake(out Command? command, TunnelTimeoutMilliseconds))
                        {
                            ...
                            {
                                Program.Log(
                                    $"[{writeFileShortName}] Preparing to send packet number {command.PacketNumber} ({command.GetName()})");
                            }
                            ...
                            if (PurgeSizeInBytes > 0 && fileStream.Position + hashingMemoryStream.Length >=
                                PurgeSizeInBytes - MESSAGE_WRITE_POS)
                            {
                                ...
                                {
                                    Program.Log(
                                        $"[{writeFileShortName}] Waiting for counterpart to be ready for purge.");
                                }
                                ...
                                {
                                    Program.Log(
                                        $"[{writeFileShortName}] Waiting for counterpart clear their ready flag.");
                                }
                            ...
                            {
                                Program.Log(
                                    $"[{writeFileShortName}] Wrote packet number {command.PacketNumber} ({command.GetName()}) to position {commandStartPos:N0} - {commandEndPos:N0} ({(commandEndPos - commandStartPos).BytesToString()})");
                            }
                    ...
                    {
                        Program.Log(
                            $"[{writeFileShortName}] Serialised {commandsSent:N0} commands into {Path.GetFileName(WriteToFilename)} ({bytesWritten.BytesToString()})");
                    }
            ...
            try
            {
                ...
                try
                {
                    ...
                    Stream isReadyForPurgeStream = IsolatedReads
                        ? new IsolatedReadsFileStream(ReadFromFilename)
                        : new FileStream(ReadFromFilename, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, 1,
                ...
                while (true)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (checkForSessionChange.ElapsedMilliseconds > 1000)
                        {
                            ...
                            {
                                Program.Log(
                                    $"[{readFileShortName}] waiting for data at position {fileStream.Position:N0}.");
                            }
                    ...
                    {
                        var exMsg =
                            $"[{readFileShortName}] Could not read command at file position {commandStartPos:N0}. [{ReadFromFilename}]";
                        if (Verbose)
                        {
                            Program.Log(exMsg);
                        }
                    ...
                    {
                        Program.Log(
                            $"[{readFileShortName}] Received packet number {command.PacketNumber} ({command.GetName()}) from position {commandStartPos:N0} - {commandEndPos:N0} ({(commandEndPos - commandStartPos).BytesToString()})");
                    }
