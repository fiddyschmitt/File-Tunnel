...
public static class SocksNegotiator
{
    ...
    private static SocksRequest ReadSocks5(Stream client)
    {
    ...
    private static SocksRequest ReadSocks4(Stream client)
    {
    ...
    private static (string Host, byte[] Addr) ReadAddress(Stream client, byte atyp)
    {
        switch (atyp)
        {
            case 0x01:   // IPv4
    ...
    private static string FormatHost(byte atyp, byte[] addr) => atyp switch
    {
    ...
    private static IPEndPoint? TryBuildDeclaredEndpoint(byte atyp, byte[] addr, int port)
    {
    ...
    private static void WriteSocks5Reply(Stream client, byte rep)
    {
    ...
    private static void WriteSocks4Reply(Stream client, byte cd)
    {
        client.Write([0x00, cd, 0, 0, 0, 0, 0, 0]);
        client.Flush();
    }

    private static int ReadPort(Stream client)
    {
        var buf = new byte[2]; client.ReadExactly(buf);
        return (buf[0] << 8) | buf[1];   // big-endian
    }

    private static byte ReadByteStrict(Stream client)
    {
    ...
    private static string ReadNullTerminated(Stream client)
    {
